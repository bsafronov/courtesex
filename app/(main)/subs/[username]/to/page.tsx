import { getUserFollowers } from "@/actions/get-user-followers";
...
export default async function Page({ params: { username } }: Props) {
  ...
  if (followers.length === 0) {
    return (
      <div className="flex flex-col gap-2">
        <p className="mb-16 text-muted-foreground">Подписки отсутствуют</p>
  ...
  return (
    <div>
      {followers.map(({ sourceUser: { id, avatar, username } }) => (
